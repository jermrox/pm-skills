...
All 71 skills become available immediately, no clone required. Invoke any skill by name, like `/pm-skills:deliver-prd "Feature description"`. Already installed via the old `pm-skills-marketplace`? It keeps working, no action needed - see [Setup by Platform](platforms.md) to move to the new home.
...
Installs all 71 skills into your agent's default skills directory via the open [`skills` CLI](https://github.com/vercel-labs/skills). No clone, no sync.
...
- **71 skills in `skills/`** covering the PM lifecycle (30 phase + 11 foundation + 15 utility + 15 tool)
...
- Lists all 71 registered skills with paths and descriptions
- Provides workflows
