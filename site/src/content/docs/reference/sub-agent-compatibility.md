...
| pm-figjam-facilitator (added unreleased) | None (invoke by name; the three FigJam skills are invoked directly) | UNVERIFIED - not yet smoke-tested. Declares the `Skill` tool to delegate to the FigJam skills, so it depends on the same native `Skill`-from-sub-agent path pm-workflow-orchestrator exercises. Board authoring additionally requires a connected Figma MCP server | UNVERIFIED - requires a connected Figma MCP server | UNVERIFIED - requires a connected Figma MCP server |
