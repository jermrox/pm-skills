...
|---|---|---|---|---|
| WS-1 | C-1 ratification packet: reconfirm the F-54 (memory-aware cohort) provisional ID against live issues; ratify parked D1 (structure-over-prose); record the D3-C write posture; lock the cohort membership (parked D3); author the 1-2 page ledger delta spec (execution state, artifact hashes, provenance chains) EXTENDING `../_unreleased/project-memory/spec_project-memory.md` | - | claude drafts, human ratifies | Delta spec committed; parked plan annotated PROMOTED, not rewritten |
...
| WS-3 | C-1 B2 cohort: read/write contracts on the locked 4-7 skills under the D3-C posture; per-skill MINOR bumps + HISTORY rows + derived-surface regen | WS-2 | claude | Cohort skills read/write memory; all gates green |
