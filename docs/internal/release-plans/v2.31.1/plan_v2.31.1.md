...
**Status:** **READY TO TAG** (scope fully merged to main; gates G0-G4 executing 2026-07-30).
**Owner:** Maintainers
...
| D2 | **G1 codex adversarial review is waived** for this tag | DECIDED-with-rationale. Every scope item shipped through an individually CI-verified PR on the fixed (always-reporting) required-check regime; the two authored fixes carry verification evidence in their PR bodies (including a negative test for the validator fix). Deterministic tooling + deps PATCH, no skill/behavior change: the v2.27.1 D2 precedent applies. Flagged, not hidden. Maintainer attests at the G1 prompt. |
