...
|---|---|---|---|
| WS-Z7 (memory, R-22 / F-48 project state) | v2.31.0 shipped + the orchestrator artifact-ledger interface agreed | File/update a tracking issue linking [`plan_project-memory.md`](../v2.32.0/plan_project-memory.md) (then in `_unreleased/project-memory/`; shipped with v2.32.0 and re-homed there 2026-08-17); note the ledger + provenance delta (X-3, artifact provenance/upgrade loop) as one paragraph | Edit the parked plan; write any memory-file code or schema; touch the orchestrator |
