...
v2.30.0 (trust repair) has no direct relationship to this bet. The project-memory plan's B2 cohort (shipped with v2.32.0; the plan now lives at `docs/internal/release-plans/v2.32.0/plan_project-memory.md`) carries loosely-typed provenance tags (observation, interpretation, hypothesis, decision) on memory entries, a precedent for structured metadata, but it is not a schema and does not overlap this bet's scope; X-03 (artifact provenance, the sibling document in this same set) integrates with memory more directly than X-02 does.
