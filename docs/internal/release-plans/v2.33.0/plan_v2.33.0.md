...
| WS-8 | **D10 (C-8) RULED A 2026-08-17 and executed; D9 (C-3) still awaiting its ruling.** Brief prepared 2026-08-16; both items are maintainer-only, so this workstream stopped at a decision-ready brief: see [WS-8 decision brief](#ws-8-decision-brief-prepared-2026-08-16-d10-ruled-2026-08-17-d9-open) below, carrying D9 (C-3) and D10 (C-8) with context, options, a recommendation, the case against each recommendation, and a `Final decision` line (D10's now filled; D9's still blank). **C-3's shape changed on evidence collected after it was scoped:** the failure it was meant to prevent *passed* the existing lint, so promotion alone would not have caught it. **C-8's premise changed too:** [#223](https://github.com/product-on-purpose/pm-skills/issues/223)'s substance shipped in v2.32.0, making this a disposition rather than a build call | D5 | human | C-3 and C-8 ruled and recorded |
...
## WS-8 decision brief (prepared 2026-08-16; D10 ruled 2026-08-17, D9 open)

Both items are maintainer-only per the agent-assignment framework. Prepared here with the evidence and a recommendation so each can be ruled in one pass. **D10 was ruled 2026-08-17** (see its Final decision line); **D9 remains open** and its Final decision line stays deliberately blank.
